...
const ORDEN_FORM_INICIAL: OrdenForm = {
  ...
  origenSolicitud: 'MANUAL_SUPERVISOR',
  descripcionProblema: '',
}
...
export default function SupervisorDashboard() {
  ...
  const actionItems = useMemo(
    () => [
      { label: 'Crear orden', targetId: 'sup-orden-manual', tone: 'primary' as const },
      { label: 'Asignar visita', targetId: 'sup-crear-visita', tone: 'primary' as const },
      { label: 'Crear actividad', targetId: 'sup-crear-actividad', tone: 'primary' as const },
      { label: 'Nuevo cliente', href: '/portal/reportes?abrir=cliente&context=dashboard&from=supervisor' },
      { label: 'Nueva máquina', href: '/portal/reportes?abrir=maquina&context=dashboard&from=supervisor' },
      { label: 'Solicitudes', targetId: 'sup-solicitudes' },
      { label: 'Visitas', targetId: 'sup-visitas' },
      { label: 'Mensajes', targetId: 'sup-mensajes' },
  ...
  const visitasFiltradas = useMemo(() => (dashboard?.visitas || []).filter((visita) => cumpleFiltroVisita(visita.estado, filtroVisitas)), [dashboard?.visitas, filtroVisitas])
  ...
  async function enviarOrdenManual(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const orden = await crearOrdenServicio({
        ...
        origenSolicitud: ordenForm.origenSolicitud || 'MANUAL_SUPERVISOR',
      ...
      <div className="dashboardContainer roleDashboardContainer">
        ...
        <section className="dashboardHero roleHeroCard" id="sup-resumen">
          <div className="dashboardHeroText">
            <h2>Supervisión de solicitudes, órdenes, visitas y reportes</h2>
            <p>{loading ? 'Cargando métricas del sistema...' : 'Base funcional para crear órdenes, asignar técnicos, revisar solicitudes y controlar reportes sin cierre.'}</p>
          </div>
        ...
        <section className="dashboardGrid roleDashboardGrid">
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-orden-manual" open={panelActivo === 'orden'}>
            <summary
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-crear-visita" open={panelActivo === 'visita'}>
            <summary
              className="operationAccordionHeader"
              onClick={(event) => {
                event.preventDefault()
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-crear-actividad" open={panelActivo === 'actividad'}>
            <summary
              className="operationAccordionHeader"
              onClick={(event) => {
                event.preventDefault()
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-solicitudes">
                      ...
                      <div className="inlineActionRow">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            window.setTimeout(() => document.getElementById('sup-orden-manual')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 60)
                          }}
                        >
                          Elaborar orden
                        </button>
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-visitas">
            <summary className="operationAccordionHeader">
              <span>
                ...
                <div className="dataList scrollArea scrollAreaWide">
                  {visitasFiltradas.map((visita) => (
                    <article key={visita.id} className="dataListItem dataListItemStack">
                      ...
                      <div className="inlineActionRow">
                        <a className="miniActionButton" href={`/portal/reportes?abrir=detalle-visita&context=dashboard&from=supervisor&visitaId=${visita.id}`}>Ver detalle</a>
                        {(visita.reportes || []).length ? (
                          <a className="miniActionButton" href={`/portal/reportes?abrir=detalle&context=dashboard&from=supervisor&reporteId=${visita.reportes?.[0]?.id}`}>Ver reporte</a>
                        ) : (
                          <a className="miniActionButton miniActionPrimary" href={`/portal/reportes?abrir=crear&context=dashboard&from=supervisor&visitaId=${visita.id}`}>Crear reporte</a>
                        )}
                      </div>
                    </article>
                  ))}
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-mensajes">
            <summary className="operationAccordionHeader">
              <span>
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-mis-actividades">
          ...
          <details className="moduleCard moduleCardFullRow operationAccordion" id="sup-reportes">
            ...
            <div className="operationAccordionBody">
              ...
              {reportesFiltrados.length ? <div className="reportGridTwo">{reportesFiltrados.map((reporte) => <article key={reporte.id} className="dataListItem dataListItemStack"><div><strong>{reporte.numeroReporte}</strong><p>{reporte.cliente?.nombre || 'Cliente'} · {formatearFecha(reporte.fechaReporte)}</p></div><div className="dataTags"><span className="statusPill">{reporteCerrado(reporte) ? 'CERRADO' : 'SIN CIERRE'}</span><span className="statusPill">{reporte.estado || 'EMITIDO'}</span></div><div className="inlineActionRow"><a className="miniActionButton" href={reporte.acciones?.verPdf || `/portal/reportes?abrir=detalle&context=dashboard&from=supervisor&reporteId=${reporte.id}`} target={reporte.acciones?.verPdf ? '_blank' : undefined} rel="noreferrer">Ver reporte</a><a className="miniActionButton miniActionPrimary" href={`/portal/reportes?abrir=detalle&context=dashboard&from=supervisor&reporteId=${reporte.id}`}>Gestionar cierre</a></div></article>)}</div> : <div className="emptyBlock">No hay reportes para el filtro seleccionado.</div>}
            </div>
          </details>
        </section>
      </div>
